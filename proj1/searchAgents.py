...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        # Those 4 flags represent whether each corner has been visited, respectively
        start_state = {"Pos": self.startingPosition,
                       "CornerFlag": [False, False, False, False]}
        return start_state
    ...
    def isGoalState(self, state):
        ...
        """
        for is_visited in state["CornerFlag"]:
            if not is_visited:
                return False
        return True
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                new_pos = (nextx, nexty)
                new_corner_flag = state["CornerFlag"][:]
                for i, corner in enumerate(self.corners):
                    if new_pos == corner:
                        new_corner_flag[i] = True
                new_state = {"Pos": new_pos,
                             "CornerFlag": new_corner_flag}
